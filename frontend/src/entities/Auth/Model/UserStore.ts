import { IUser } from "../../../shared/api/models/IUser";
...
export default class Store {
    ...
    async updateUserData(name: string, email:string, currentPassword:string, newPassword:string, newPassword_confirmation:string) {
        try {
            const response = await AuthService.updateUserData(name, email, currentPassword, newPassword, newPassword_confirmation);
            localStorage.setItem('token', response.data.accessToken);
            this.setAuth(true)
            this.setUser(response.data.user);
        } catch (e: any) {
